...
def train(env, episodes):
    ...
    for episode in range(1, episodes + 1):
        ...
        while True:
            ...
            # log info
            if global_timestep % 1000 == 0:
                tqdm.write(f"Episode {episode}, Step {global_timestep}, Loss: {loss:.4f}, Epsilon: {epsilon}")
        ...
        if ret > max_ret:
            agent.save_model(episode, file_name='policy_model_best_v2.pth')
            max_ret = ret

        # Save model every 50 episodes
        if episode % 20 == 0:
            agent.save_model(episode, file_name='policy_model_latest_v2.pth')
...
def save_plot(episode, losses, rewards, qvalues, returns):
    ...
    fig.tight_layout()
    plt.savefig(f"plot/training4/episode-{episode}.png")
